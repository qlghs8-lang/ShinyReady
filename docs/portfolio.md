...
용량 문제로 저장소에는 포함하지 않았다. 아래 플레이 영상을 Notion/노션 페이지나 별도 포트폴리오 사이트에 업로드해 함께 첨부할 것.

| 파일 | 위치 |
|------|------|
| ShinyReady_PlayGIF_01 ~ 07 | `바탕화면/ShinyReady_PlayGIF/` (총 7개, 4.7MB~47.9MB) |

각 GIF에 대해 어떤 기능을 보여주는지 간단한 캡션을 붙여 첨부하면 좋다 (예: "구역 해금 → 직원 자동화", "광고 버프 적용 후 세차 속도 변화" 등).
